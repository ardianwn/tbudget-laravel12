...
@section('title', 'Edit Transportasi - ' . $transportation->route_name)
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                ...
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mt-2">
                    Edit Transportasi: <span class="text-blue-600">{{ $transportation->route_name }}</span>
                </h1>
            </div>
            <div class="flex items-center space-x-2">
                <form action="{{ route('admin.transportations.destroy', $transportation) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus transportasi ini?')">
                    @csrf
                    @method('DELETE')
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <form action="{{ route('admin.transportations.update', $transportation) }}" method="POST" id="transportationForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="route_name" id="route_name" 
                                   value="{{ old('route_name', $transportation->route_name) }}" required
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="train" {{ old('type', $transportation->type) == 'train' ? 'selected' : '' }}>Kereta Api</option>
                                <option value="minibus" {{ old('type', $transportation->type) == 'minibus' ? 'selected' : '' }}>Minibus</option>
                                <option value="ferry" {{ old('type', $transportation->type) == 'ferry' ? 'selected' : '' }}>Kapal Ferry</option>
                                <option value="airplane" {{ old('type', $transportation->type) == 'airplane' ? 'selected' : '' }}>Pesawat</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-xs italic mt-1 flex items-center">
                                    <i class="fas fa-exclamation-circle mr-1"></i> {{ $message }}
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="price" id="price" 
                                       value="{{ old('price', $transportation->price) }}" min="0" required
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="relative">
                                <input type="number" name="duration_minutes" id="duration_minutes" 
                                       value="{{ old('duration_minutes', $transportation->duration_minutes) }}" min="1" required
                                       class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition duration-200 px-4 py-2">
                        ...
                        <div class="md:col-span-2 space-y-2">
                            ...
                            @php
                                $timesValue = old('departure_times', $transportation->departure_times ? implode(', ', $transportation->departure_times) : '');
                            @endphp
                        ...
                        <!-- Koordinat Rute - Improved Version -->
                        <div class="md:col-span-2 bg-gray-50 p-4 rounded-lg">
                            ...
                            <div class="space-y-4" id="coordinates-container">
                                @php
                                    $coordinates = old('coordinates', $transportation->coordinates ?? []);
                                    if (empty($coordinates)) {
                                        $coordinates = [['lat' => '', 'lng' => '']]; // Default empty coordinate
                                    }
                                @endphp
